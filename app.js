...
const App = {
  ...
  setAuthBusy(busy, label = 'Tiếp tục') {
    const submitBtn = document.querySelector('#auth-overlay .btn-primary');
    const resetBtn = document.querySelector('#auth-overlay .btn-secondary:last-of-type');
    if (submitBtn) {
  ...
  async processAuthenticatedUser(user, source = 'listener') {
    try {
      ...
      try {
        ...
        this.setSyncStatus('Đã sẵn sàng');
        this.showAuthMessage('Đăng nhập thành công.');
      } catch (cloudError) {
        console.error(cloudError);
        this.showAuthMessage('Đăng nhập thành công nhưng tải dữ liệu đám mây chậm. Web đang dùng dữ liệu cục bộ.', true);
  ...
  switch(mode) {
    ...
    App.showAuthMessage(
      mode === 'login'
        ...
        : 'Tạo tài khoản mới để bắt đầu sử dụng hệ thống giao dịch của bạn.'
    );
  },
