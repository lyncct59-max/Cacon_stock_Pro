...
const App = {
  ...
  initLanding() {
    if (this.state.landingReady) return;
    this.state.landingReady = true;
    const canvas = document.getElementById('landing-canvas');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const palette = ['#22c55e', '#3b82f6', '#f59e0b', '#ef4444', '#10b981'];
    ...
    const count = Math.min(110, Math.max(70, Math.floor(window.innerWidth / 18)));
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 135) {
            ...
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ctx.strokeStyle = `rgba(148,163,184,${(1 - d / 135) * 0.18})`;
            ctx.lineWidth = 1;
            ctx.stroke();
          }
        }
        if (mouse.active) {
          ...
          if (d < 170) {
            ...
            ctx.strokeStyle = `rgba(255,255,255,${(1 - d / 170) * 0.22})`;
            ctx.stroke();
          }
        }
        ctx.beginPath();
  ...
  updateAppLock() {
    const locked = this.state.landingVisible || !this.state.user;
    document.querySelector('.app-shell').classList.toggle('app-locked', locked);
    document.getElementById('auth-overlay').classList.toggle('hidden', this.state.landingVisible || !!this.state.user);
  },

  dismissLanding(openAuth = true) {
    this.state.landingVisible = false;
    document.getElementById('landing-screen')?.classList.add('hidden');
    this.updateAppLock();
    if (!this.state.user && openAuth) document.getElementById('auth-email')?.focus();
  },

  showLanding() {
    this.state.landingVisible = true;
  ...
  async logout() { this.showLanding(); return FirebaseService.logout(); },
  ...
  saveMarket() {
    if (!this.state.user) return alert('Bạn cần đăng nhập để lưu dữ liệu thị trường lên Firebase.');
    this.data.market = {
  ...
  saveMindset() {
    if (!this.state.user) return alert('Bạn cần đăng nhập để lưu check-in lên Firebase.');
    this.data.mindset = {
  ...
  saveReview() {
    if (!this.state.user) return alert('Bạn cần đăng nhập để lưu review lên Firebase.');
  ...
  async saveTrade() {
    try {
      if (!this.state.user) throw new Error('Bạn cần đăng nhập để lưu nhật ký lên Firebase.');
      ...
      const rawResult = document.getElementById('trade-result').value;
      const finalStatus = rawResult === 'open' ? 'open' : 'closed';
      const finalExitDate = finalStatus === 'closed' ? document.getElementById('trade-exit-date').value : '';
      const finalExit = finalStatus === 'closed' && document.getElementById('trade-exit').value ? Number(document.getElementById('trade-exit').value) : null;
      const obj = {
  ...
  saveWatchlist() {
    if (!this.state.user) return alert('Bạn cần đăng nhập để lưu watchlist lên Firebase.');
  ...
  async savePattern() {
    try {
      if (!this.state.user) throw new Error('Bạn cần đăng nhập để lưu mẫu hình lên Firebase.');
