...
const FirebaseService = {
  ...
  async uploadFile(uid, file, folder) {
    if (!file) return '';
    if (!(window.storage && uid)) return this.fileToDataUrl(file);
    try {
      ...
      console.warn('Upload Firebase Storage thất bại, dùng ảnh local base64 thay thế.', error);
...
const App = {
  ...
  bindEvents() {
    ...
    document.getElementById('trade-result').addEventListener('change', () => this.syncTradeCloseFields('result'));
    document.getElementById('trade-status').addEventListener('change', () => this.syncTradeCloseFields('status'));
  },
  ...
  normalizeTradeStatusAndResult(trade) {
    const next = { ...trade };
    if (next.result === 'open') next.status = 'open';
    if (next.result === 'win' || next.result === 'loss') next.status = 'closed';
    if (next.status === 'open') {
      next.result = 'open';
      next.exit = null;
      next.exitDate = '';
    }
    if (next.status === 'closed' && next.result === 'open') {
      next.result = (next.exit != null && next.entry != null && Number(next.exit) >= Number(next.entry)) ? 'win' : 'loss';
    }
    return next;
  },

  deriveResultLabelFilter(trade, resultFilter) {
    if (resultFilter === 'all') return true;
    if (resultFilter === 'closed') return trade.status === 'closed';
  ...
  journalFilteredTrades() {
    ...
    return this.data.trades.filter(t => {
      ...
      const hitResult = this.deriveResultLabelFilter(t, result);
  ...
  syncTradeCloseFields(source = 'result') {
    ...
    if (source === 'result') {
      if (resultEl.value === 'open') {
        statusEl.value = 'open';
        exitEl.value = '';
        exitDateEl.value = '';
      } else {
        statusEl.value = 'closed';
        if (!exitDateEl.value) exitDateEl.value = new Date().toISOString().slice(0, 10);
      }
    }

    if (source === 'status') {
      if (statusEl.value === 'open') {
        resultEl.value = 'open';
        exitEl.value = '';
        exitDateEl.value = '';
      } else if (statusEl.value === 'closed' && resultEl.value === 'open') {
        const entry = Number(document.getElementById('trade-entry').value || 0);
        const exit = document.getElementById('trade-exit').value ? Number(document.getElementById('trade-exit').value) : null;
        resultEl.value = exit != null && exit < entry ? 'loss' : 'win';
        if (!exitDateEl.value) exitDateEl.value = new Date().toISOString().slice(0, 10);
      }
    }
  },
  ...
  openTradeModal(id = null, prefill = {}) {
    ...
    this.syncTradeCloseFields('result');
  ...
  async saveTrade() {
    try {
      ...
      const uploadUid = this.state.user?.uid || 'local-user';
      if (theoryFile) theoryImage = await FirebaseService.uploadFile(uploadUid, theoryFile, 'trades/theory');
      if (actualFile) actualImage = await FirebaseService.uploadFile(uploadUid, actualFile, 'trades/actual');
      ...
      obj = this.normalizeTradeStatusAndResult(obj);
